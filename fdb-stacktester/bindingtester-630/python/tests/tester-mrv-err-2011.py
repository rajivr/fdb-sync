...
class Tester:
    ...
    def run(self):
        for idx, i in enumerate([
                ...
                (six.u('NEW_TRANSACTION'),), \
                (six.u('GET_READ_VERSION'),), \
                (six.u('COMMIT'),), \
                (six.u('WAIT_FUTURE'),), \
                ...
                (six.u('COMMIT'),), \
                (six.u('WAIT_FUTURE'),), \
                (six.u('GET_COMMITTED_VERSION'),), \
                (six.u('RESET'),), \
                (six.u('SET_READ_VERSION'),), \
                (six.u('CANCEL'),), \
                (six.u('GET_READ_VERSION'),), \

                # # From binding tester bisect
                # (six.u('NEW_TRANSACTION'),), \
                # (six.u('COMMIT'),), \
                # (six.u('WAIT_FUTURE'),), \
                # (six.u('GET_COMMITTED_VERSION'),), \
            ...
            print("Tester.tr_map: %r" % Tester.tr_map)
            print("last_version: %r" % self.last_version)
...
if __name__ == '__main__':
    db = fdb.open()
    t = Tester(db, "test_spec".encode('ascii'))
    t.run()
