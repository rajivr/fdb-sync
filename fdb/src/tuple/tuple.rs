...
impl Tuple {
    ...
    pub fn from_bytes(b: Bytes) -> FdbResult<Tuple> {
        ...
        let _ = i64::try_from(i.clone())
		    ...
		    // `TupleValue::NegativeArbitraryPrecisionInteger`.
			    ...
			    panic!("Byte encoded length of BigInt *must* be less than or equal to 255.");
                    ...
                    if ((BigInt::parse_bytes(b"9223372036854775808", 10).unwrap())
                ...
                .and_then(|x| match x {
                        ...
                        if (72057594037927936..=9223372036854775808).contains(i) =>
                    {
                        Some(
                            ...
                            i64::try_from(Into::<i128>::into(*i) * -1).unwrap(),
                        )
                    }
                    ...
                    &TupleValue::NegInt6(ref i)
                        if (1099511627776..=281474976710655).contains(i) =>
                    {
                        Some(
                            // Safe to unwrap here because we are
                            // checking for the range.
            ...
            self.elements
                ...
                .and_then(|x| match x {
                    &TupleValue::NegInt4(ref i) if (16777216..=2147483648).contains(i) => Some(
                        // Safe to unwrap here because we are
                        // checking for the range.
                        i32::try_from(Into::<i64>::into(*i) * -1).unwrap(),
                    ),
            ...
            self.elements
                ...
                .and_then(|x| match x {
                    &TupleValue::NegInt2(ref i) if (256..=32768).contains(i) => Some(
                        // Safe to unwrap here because we are
                        // checking for the range.
                        i16::try_from(Into::<i32>::into(*i) * -1).unwrap(),
                    ),
                    &TupleValue::NegInt1(ref i) if (129..=255).contains(i) => {
                        Some(Into::<i16>::into(*i) * -1)
                    }
            ...
            .and_then(|x| match x {
                &TupleValue::NegInt1(i) if i <= 128 => {
                    Some(
                        // Safe to unwrap here because we are checking
                        // for the range.
                        i8::try_from(Into::<i16>::into(i) * -1).unwrap(),
                    )
                }
...
mod tests {
    ...
    fn add_tuple() {
        ...
        assert_eq!(t.has_incomplete_versionstamp(), true);
    ...
    fn add_versionstamp() {
        ...
        assert_eq!(t.has_incomplete_versionstamp(), false);
    ...
    fn get_bool() {
        ...
        assert_eq!(t.get_bool(0).unwrap_err(), Tuple::tuple_get_error());
        assert_eq!(t.get_bool(1).unwrap(), true);
        assert_eq!(t.get_bool(2).unwrap(), false);
    }
